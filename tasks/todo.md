...
# 当前任务（2026-06-02 win.csv 称号列数修复）

- [x] 按表头列数检查 `prizedata/win.csv`
- [x] 对列数异常行压缩已有称号并补齐空列
- [x] 复查所有行列数一致
- [ ] 记录结果并提交
...
- 表头是 `职业 + 1..18`，每行必须是 `19` 列。
- 对缺少或多出逗号的异常行，优先让已有称号向前移动，降低获取难度。
- 列数本来正确的行不主动压缩，避免扩大影响面。

## Review

- 修复前 `win.csv` 共 `223` 行，其中 `146` 行列数不足，列数分布为 `13/14/15/16/17/18/19`。
- 修复策略：仅处理列数异常行，保留职业 ID，把后续非空称号按原顺序前移，再补空字段到 `19` 列。
- 修复后 `223` 行全部为 `19` 列，没有多列或少列。
